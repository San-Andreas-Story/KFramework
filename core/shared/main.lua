...
KFramework.toInternal = function(event, ...)
    KFramework.logDev(("Envoie d'un event interne ^6>^3 %s^7"):format(event))
    TriggerEvent(KFramework.hs256(event), ...)
end

--- (G - Core/Shared) Déclenche un événement interne (Execute même coté) en clair (sans obfuscation du nom).
---@param event string Le nom de l'événement interne à déclencher.
---@param ... any Les arguments optionnels à passer à l'événement.
KFramework.toInternalExposed = function(event, ...)
    KFramework.logDev(("Envoie d'un event interne ^6>^3 %s^7"):format(event))
    TriggerEvent(event, ...)
end
...
KFramework.onReceive = function(event, handler)
    local baseEvent = event
    event = KFramework.hs256(event)
...
---@param event string Le nom clair de l'événement à écouter.
---@param handler function La fonction de rappel (callback) à exécuter lors du déclenchement.
KFramework.onReceiveExposed = function(event, handler)
    if (not (isEventRegistred(event))) then
        registerEvent(event)
    end
    AddEventHandler(event, handler)
...
---@param handler function La fonction de rappel (callback) à exécuter lors du déclenchement.
KFramework.onReceiveWithoutNetExposed = function(event, handler)
    AddEventHandler(event, handler)
end
...
---@param event string Le nom clair de l'événement à écouter.
---@param handler function La fonction de rappel (callback) à exécuter lors du déclenchement.
KFramework.onReceiveWithoutNet = function(event, handler)
    event = KFramework.hs256(event)
    AddEventHandler(event, handler)
end
...
KFramework.hash = function(string)
...
KFramework.hs256 = function(string)
...
---@param stringValue string Le message à afficher dans les logs.
KFramework.logOverall = function(stringValue)
    print(("%s %s^7"):format(_Config.prefix, stringValue))
end
...
KFramework.SQL = function(SQLValue)
...
KFramework.Error = function(stringValue)
...
KFramework.Succes = function(stringValue)
